...
class _HttpClientResponse extends _HttpInboundMessageListInt implements HttpClientResponse {
  ...
  _HttpClientResponse(
      _HttpIncoming _incoming, this._httpRequest, this._httpClient, this._profileData)
...
abstract class _HttpOutboundMessage<T> extends _IOSinkImpl {
  ...
  _HttpOutboundMessage(
      Uri uri, String protocolVersion, _HttpOutgoing outgoing, _HttpProfileData? profileData,
      {_HttpHeaders? initialHeaders})
      : _uri = uri,
        headers = _HttpHeaders(protocolVersion,
            defaultPortForScheme:
                uri.isScheme('https') ? HttpClient.defaultHttpsPort : HttpClient.defaultHttpPort,
...
class _HttpClientConnection {
      ...
      if (incoming.statusCode == 100) {
        incoming.drain().then((_) {
          ...
          _nextResponseCompleter!
              .completeError(HttpException(error.message, uri: _currentUri), stackTrace);
  ...
  _HttpClientRequest send(
      Uri uri, int port, String method, _Proxy proxy, _HttpProfileData? profileData) {
    if (closed) {
  ...
  Future<_HttpClientConnection> createProxyTunnel(String host, int port, _Proxy proxy,
    ...
    _HttpClientRequest request =
        send(Uri(host: host, port: port), port, method, proxy, proxyProfileData);
    if (proxy.isAuthenticated) {
      // If the proxy configuration contains user information use that
      // for proxy basic authorization.
      String auth = base64Encode(utf8.encode("${proxy.username}:${proxy.password}"));
...
class _ConnectionTarget {
  ...
  _ConnectionTarget(
      this.key, this.host, this.port, this.isSecure, this.context, this.connectionFactory);
  ...
  Future<_ConnectionInfo> connect(Uri uri, String uriHost, int uriPort, _Proxy proxy,
    ...
    if (cf != null) {
      ...
      connectionTask = (isSecure && proxy.isDirect
          ? SecureSocket.startConnect(host, port,
              context: context, onBadCertificate: callback, keyLog: client._keyLog)
...
class _HttpClient implements HttpClient {
  ...
  Future<_HttpClientRequest> _openUrl(String method, Uri uri) {
    ...
    return _getConnection(uri, uri.host, port, proxyConf, isSecure, profileData).then(
  ...
  Future<_ConnectionInfo> _getConnection(Uri uri, String uriHost, int uriPort,
      _ProxyConfiguration proxyConf, bool isSecure, _HttpProfileData? profileData) {
...
class _HttpClientDigestCredentials extends _HttpClientCredentials
  ...
  void authorize(_Credentials credentials, HttpClientRequest request) {
    request.headers.set(
        HttpHeaders.authorizationHeader, authorization(credentials, request as _HttpClientRequest));
  }
