...
class _NineGridViewState extends State<NineGridView> {
  ...
  Rect _initSize(BuildContext context) {
    ...
    double itemW;
    if (widget.type == NineGridType.weiBo && (itemCount == 1 || itemCount == 2 || itemCount == 4)) {
      itemW = (width - space) / 2;
    } else {
  ...
  Widget _buildWeChatGroup(BuildContext context) {
    ...
    for (int i = 0; i < itemCount; i++) {
      double left;
      if (first > 0 && i < first) {
        left = (width - itemW * first - space * (first - 1)) / 2 + (itemW + space) * i;
      } else {
        left = (space + itemW) * ((i - first) % column);
      }

      int itemIndex =
          (first > 0 && i < first) ? 0 : (first > 0 ? (i + column - first) : i) ~/ column;

      double top = (width - itemW * row - space * (row - 1)) / 2 + (space + itemW) * itemIndex;
  ...
  Widget _buildDingTalkGroup(BuildContext context) {
    ...
    for (int i = 0; i < itemCount; i++) {
      children.add(Positioned(
          ...
          left: (widget.space + itemW) * (((itemCount == 3 && i == 2) ? i + 1 : i) % 2),
          child: SizedBox(
            width: itemCount == 1 ? width : itemW,
            height:
                (itemCount == 1 || itemCount == 2 || (itemCount == 3 && i == 0)) ? width : itemW,
            child: widget.itemBuilder(context, i),
          )));
    }
    return ClipOval(
  ...
  Widget _buildQQGroup(BuildContext context) {
    ...
    switch (itemCount) {
      ...
      case 3:
        ...
        double R = r * (1 + math.sin(math.pi / itemCount)) / math.sin(math.pi / itemCount);
...
class QQClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    if (total == 2 && index == initIndex) {
      ...
      double spaceA =
          arcAngle > 0 ? (arcAngle / 2) : (math.acos((r - math.min(r, space)) / r) / math.pi * 180);
      ...
      double spaceB = math.atan(r * math.sin(d2r(spaceA)) / (2 * r - r * math.cos(d2r(spaceA)))) /
          math.pi *
          180;
